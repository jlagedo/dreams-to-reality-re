...
```mermaid
flowchart LR
    DAT["DREAMS.DAT project + OBJET records"] --> Spawn["FUN_0041f9db / FUN_0041deb8"]
    Spawn --> Actor["Runtime actor and AI fields"]
    Actor --> Schedule["FUN_00415109 AI scheduler"]
    Schedule --> Decide["target, range, and random action choice"]
    Decide --> Request["actor +0x160 requested action"]
    Request --> Resolve["FUN_004058d5 state-to-clip lookup"]
...
- **Movement-to-animation table:** yes. `0x004f7728` is a 16-family × 64-action
  ...
  playback flags.
- **AI chain/schedule:** yes. `FUN_004115a1` groups eligible actors into
  runtime controllers, and `FUN_0041208f` applies a project-selected mode
  transition list.
- **AI layer:** in `WINDREAM.EXE`, the per-frame dispatcher is `FUN_00415109`;
  target collection and decisions run in `FUN_004131a4`, `FUN_00414d61`, and
  `FUN_00414646`.
...
`FUN_004240ba` calls `FUN_00415109`. The scheduler walks several active actor
lists in a fixed order: **[verified]**

| List | Count | Pass | Observed role |
|---|---|---|---|
| `0x005df3ec` | `0x005df474` | `FUN_0041208f` | Apply project-selected actor mode transitions. |
| `0x005df3ac` | `0x005df470` | `FUN_00413b3b` | Update linked or grouped AI actors. |
| `0x005df42c` | `0x005df46c` | `FUN_00414b24` | Run per-actor steering and action decisions. |
| `0x005df42c` | `0x005df46c` | `FUN_00414d61` | Refresh actor candidate and target state. |
| `0x005df3ac` | `0x005df470` | `FUN_00414f6d` | Aggregate status from linked actors. |

AI group-controller records store a linked-member count at `+0x19c` and member
pointers beginning at `+0x1a0`. `FUN_00410f03` increments that count, appends a
...
`FUN_004115a1` builds the lists on scene load. It scans the 32 project-actor
...
holds member actors. `FUN_00410e5d` initializes both record types. **[verified]**

`FUN_00414b24` selects work from bits in the mode word: bit `0x10` runs the
look/steering path, bit `0x02` runs the general behavior path, and bit `0x04`
runs the target action selector `FUN_00414646`. `FUN_00413b3b` applies mode
changes across an actor's linked members. **[verified]**

### Executable transition lists

`FUN_0041208f` selects a transition list from a pointer array at `0x0049d5a0`,
...
`FUN_004131a4` builds candidate lists from active actors. It sorts them by a
...
`FUN_00414d61` checks the candidate set and assigns the selected actor pointer
...
`FUN_00414646` uses the selected target, distance, facing, and a random
...
`FUN_00405118` places these requests in actor `+0x160`; the ordinary actor
...
`FUN_0041deb8` remaps selector 5 to runtime class 1 and sets actor flag
...
requests states 16, 20, 18, or 22. `FUN_00404d98` backfills missing slots from
...
The shot effect is synchronized to the attack animation. `FUN_00407b51` calls
`FUN_00442944` when the attack state's frame threshold is crossed. The effect
builder allocates a transient actor; IBI's `+0xad & 0x20` flag makes it effect
class `0x14`. `FUN_00442786` places the effect at the owner's position and
derives its velocity from the owner's orientation and `+0x10c` (IBI's value
585). `FUN_00442e0d` advances effect classes other than `0x10` by their
velocity. `FUN_00444b8f` handles effect collisions; for class `0x14`,
`FUN_004440aa` calls `FUN_00443619` to reduce the target's health at `+0x38` by
...
`FUN_0041deb8` creates an actor from a project's `OBJET` record. **[verified]**
...
| `+0x70` | Copied to actor `+0x10c`; used as attack-effect launch magnitude by `FUN_00442786`. |
...
- `FUN_0040484d` initializes the slots and assigns shared per-action flags.
- `FUN_00404d98` loads a model's `.DAN` directory, parses each `AN###` suffix,
...
- `FUN_004058d5` resolves the requested action in `+0x160` as a slot within the
...
New-game setup at `FUN_0041f699` loads the player bank from `XH_.DAN` (or
`MHE.DAN` for the alternate character), requests state 0, and enters the same
resolver. **[verified]**

For runtime class 1, `FUN_004058d5` selects an action from actor `+0x240`
divided by frame delta. Physics code `FUN_0043d360` updates `+0x238`, `+0x240`,
...
requests come from controls and AI through `FUN_00405118`.

The generic actor updater `FUN_00407b51` advances animation and movement
...
1. `FUN_00415109` schedules the NPC's mode and target passes.
2. `FUN_00414d61` chooses an eligible nearby target.
3. `FUN_00414646` checks target distance, facing, and a random threshold.
4. It requests one of action slots 16, 20, 18, or 22 through `FUN_00405118`.
...
states resolve to `IBIAN016`, then `FUN_00407b51` triggers the moving effect
through `FUN_00442944` / `FUN_00442786` / `FUN_00442e0d`; the collision path
applies damage through `FUN_00444b8f` / `FUN_004440aa` / `FUN_00443619`. The
...
is initialized at `FUN_0040484d` and populated at `FUN_00404d98`.
